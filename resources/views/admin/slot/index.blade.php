...
@section('title', 'Daftar Slot - ' . $lumbung->nama_lumbung)
...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-2xl font-semibold tracking-tight text-gray-900">Daftar Slot - {{ $lumbung->nama_lumbung }}</h1>
        <p class="text-sm text-gray-500 mt-1">Kelola slot penyimpanan gabah</p>
    </div>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($slotList as $index => $slot)
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-4 py-3 text-sm text-gray-900">{{ $slotList->firstItem() + $index }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 font-mono">{{ $slot->nama_slot }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900">{{ number_format($slot->kapasitas, 0, ',', '.') }} kg</td>
                    <td class="px-4 py-3 text-sm text-gray-900">{{ number_format($slot->kapasitas - $slot->terpakai, 0, ',', '.') }} kg</td>
                    <td class="px-4 py-3 text-sm text-gray-900">{{ number_format($slot->terpakai, 0, ',', '.') }} kg</td>
                    <td class="px-4 py-3 text-sm">
                        <div class="w-32">
                            ...
                            <p class="text-xs text-gray-500">{{ $slot->persen_terpakai }}%</p>
                        </div>
                    </td>
                    <td class="px-4 py-3 text-sm">
                ...
                @empty
                <tr>
                    <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500">Tidak ada slot</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
        <p class="text-xs text-gray-500">
            Menampilkan {{ $slotList->firstItem() }}–{{ $slotList->lastItem() }} dari {{ $slotList->total() }} data
        </p>
        {{ $slotList->links('vendor.pagination.tailwind') }}
    </div>
    @endif
</div>
