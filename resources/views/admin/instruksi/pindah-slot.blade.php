...
<nav class="flex items-center gap-2 text-sm text-gray-500">
    ...
    <span>/</span>
    <span class="text-gray-900 font-medium">Pindah Slot</span>
</nav>
@endsection
...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-semibold tracking-tight text-gray-900">Pindah Slot Lumbung</h1>
        <p class="text-sm text-gray-500 mt-1">Ubah slot penyimpanan gabah</p>
    </div>
    <a href="{{ route('admin.instruksi.show', $instruksi->id_instruksi) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors">
...
<div class="bg-indigo-50 border border-indigo-200 rounded-lg p-4 mb-6">
    <div class="flex gap-3">
        ...
        <div class="text-sm text-indigo-800">
            <p class="font-medium">Slot Saat Ini</p>
            <p class="text-indigo-700 mt-1">{{ $instruksi->slotLumbung->lumbung->nama_lumbung }} / {{ $instruksi->slotLumbung->kode_slot }}</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm">
    <div class="px-6 py-4 border-b border-gray-100">
        <h3 class="text-sm font-semibold text-gray-900 tracking-tight">Pilih Slot Alternatif</h3>
    </div>

    <form action="{{ route('admin.instruksi.pindah-slot.post', $instruksi->id_instruksi) }}" method="POST" class="p-6">
        @csrf
        ...
        <div class="space-y-3 mb-6">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="mt-2">
                            <div class="flex justify-between text-xs text-gray-500 mb-1">
                                ...
                                <span>{{ number_format($slot->kapasitas_tersedia) }} kg</span>
                            </div>
            ...
            @empty
                <div class="p-6 text-center text-gray-500">
                    <p class="text-sm">Tidak ada slot alternatif yang tersedia</p>
                </div>
            @endforelse
        </div>
        ...
        <!-- Alasan -->
        <div class="mb-6">
            <label for="alasan" class="block text-sm font-medium text-gray-700 mb-1.5">Alasan Pindah Slot</label>
            <textarea id="alasan" name="alasan" rows="4" placeholder="Jelaskan alasan pemindahan slot…" class="w-full px-3 py-2 text-sm bg-white border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-colors">{{ old('alasan') }}</textarea>
            @error('alasan')
