...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-2xl font-semibold tracking-tight text-gray-900">Daftar Lumbung</h1>
        <p class="text-sm text-gray-500 mt-1">Kelola lumbung padi dan kapasitasnya</p>
    </div>
    <a href="{{ route('admin.lumbung.create') }}"
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-gray-200 bg-gray-50">
                    ...
                    <th class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500">Pengelola</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($lumbungList as $index => $lumbung)
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-4 py-3 text-sm text-gray-900">{{ $lumbungList->firstItem() + $index }}</td>
                        ...
                        <div class="flex flex-wrap gap-1">
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-medium
                                {{ $pengelola->pivot->peran === 'pemilik_akun' ? 'bg-indigo-50 text-indigo-700' : 'bg-gray-100 text-gray-600' }}"
                                  title="{{ $pengelola->nama_pengelola }}">
                                {{ $pengelola->pivot->peran === 'pemilik_akun' ? 'Pemilik' : 'Anggota' }}
                            </span>
                            @empty
                    ...
                    <td class="px-4 py-3 text-sm">
                        <div class="flex gap-2">
                            ...
                            <form action="{{ route('admin.lumbung.destroy', $lumbung->id_lumbung) }}" method="POST" class="inline"
                                  onsubmit="return confirm('Yakin ingin menghapus lumbung ini?')">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500">Tidak ada data lumbung</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <!-- Pagination -->
    @if($lumbungList->count() > 0)
    <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
        <p class="text-xs text-gray-500">
            Menampilkan {{ $lumbungList->firstItem() }}–{{ $lumbungList->lastItem() }} dari {{ $lumbungList->total() }} data
        </p>
        {{ $lumbungList->links('vendor.pagination.tailwind') }}
    </div>
    @endif
</div>
